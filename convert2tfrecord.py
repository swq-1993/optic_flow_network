...
def convert_dataset(indices, name):
    ...
    for i in indices:
        ...
        boundary_a = imread(boundary_a_path)
        boundary_b = imread(boundary_b_path)
        ...
        boundary_a_raw = boundary_a.tostring()
        boundary_b_raw = boundary_b.tostring()
        flow_raw = open_flo_file(flow_path).tostring()

        example = tf.train.Example(features=tf.train.Features(feature={
            'image_a': _bytes_feature(image_a_raw),
            'image_b': _bytes_feature(image_b_raw),
            'boundary_a': _bytes_feature(boundary_a_raw),
            'boundary_b': _bytes_feature(boundary_b_raw),
